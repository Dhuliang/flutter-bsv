...
class MerkleNode {
  ...
  Hash hash() {
    if (this.hashBuf != null) {
      return Hash(data: this.hashBuf!.asUint8List());
    }
    if (this.buf != null) {
      return Hash.sha256Sha256(this.buf!.asUint8List());
    }
    var hashBuf1 = this.merkle1!.hash();
    var hashBuf2 = this.merkle2!.hash();
    this.buf = List<int>.from([...hashBuf1.toBuffer(), ...hashBuf2.toBuffer()]);
    return Hash.sha256Sha256(this.buf!.asUint8List());
  }
  ...
  factory MerkleNode.fromBuffers(List<List<int>> bufs) {
    return new MerkleNode().fromBuffers(bufs);
  }
  ...
  MerkleNode fromBufferArrays(List<List<int>> bufs1, List<List<int>> bufs2) {
    if (bufs1.length == 1) {
      this.merkle1 = new MerkleNode(hashBuf: null, buf: bufs1[0]);
      this.merkle2 = new MerkleNode(hashBuf: null, buf: bufs2[0]);
      return this;
    }
  ...
  factory MerkleNode.fromBufferArrays(
      List<List<int>> bufs1, List<List<int>> bufs2) {
    return new MerkleNode().fromBufferArrays(bufs1, bufs2);
  }
